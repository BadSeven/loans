 ...
 * @param {Object} options 参数对象
 */
plugins.singleImageUpload2 = function( options ) {
	var defaults = {
			list : '#fileList2',
			btn : '#btn-star2',
			pickId : '#filePicker2',
			inputBox : '#singleImageUpload2',
			state : 'pending',
		}
	var _opt = $.extend(defaults,options);
	$list = $( _opt.list ),
 ...
 * @param {Object} options 参数对象
 */
plugins.singleDemandRemove = function( options ) {
	var defaults = {
		'clickObj' : '.singleDemandRemove',
		'url' : "demandRemove",
		'event' : 'click'
	};
	var _opt = $.extend( defaults, options );
 ...
 * @param {Object} options 参数对象
 */
plugins.singleDtorjoinRemove = function( options ) {
	var defaults = {
		'clickObj' : '.singleDtorjoinRemove',
		'url' : "dtorjoinRemove",
		'event' : 'click'
	};
	var _opt = $.extend( defaults, options );
 ...
 * @param {Object} options 参数对象
 */
plugins.BatchDemandRemove = function( options ) {
	var defaults = {
		'clickObj' : '.BatchDemandRemove',
		'url' : 'demandRemove',
		'event' : 'click',
		'refreshBtn' : '#RefreshButton'
	};
...
plugins.send = function( options ) {
	...
	$( _opt.clickObj ).on( _opt.event, function() {
		...
		var _id = plugins.getCheckedId('');
		if ( _id == "" ) {
			layer.msg('请选择要发送短信的会员!',{icon: 3,time:2000});
			return;
		}
		layer.confirm('确认要为这些会员发送短信吗？',function(index){
			layer_show('钱包充值',_opt.url+'/id/'+_id,'600','350');
		});
	} );
}
 ...
 * @param {Object} options 对象节点
 */
plugins.updateStatusAll = function( options ) {
	var defaults = {
		'clickObj' : '.updateStatusAll',
		'event' : 'click'
	};
	var _opt = $.extend( defaults, options );
	...
	$( document ).on( _opt.event, _opt.clickObj, function() {
		...
		var _status = _this.attr( 'data-status' );
		var _field = _this.attr( 'data-field' );
		var confirmInfo = _status == 1 ? '确认要取消推荐在首页显示吗？' : '确认要推荐在首页显示吗？';
		layer.confirm(confirmInfo,function(index){
			$.ajax({
				url : jsObj['TheURL'] + '/commSetStatusAll/id/'+ _id +'/param/'+ _field +'-'+ _status +'.html?rnd='+ Math.random(),
				data : {},
				type : 'POST',
				async : true,
				dataType : 'json',
				...
				success : function( data ) {
					if ( data['status'] == 0 ) {
						...
						if ( _status == 0 ) {
							...
							_this.parents("tr").find(".td-manage").prepend('<a href="javascript:void(0);" class="updateStatusAll" data-id="'+ _id +'" data-status="1" data-field="is_index" title="取消推荐在首页显示"><i class="Hui-iconfont">&#xe69d;</i></a>');
							
						} else if ( _status == 1 ) {

							_this.parents("tr").find(".td-manage").prepend('<a href="javascript:void(0);" class="updateStatusAll" data-id="'+ _id +'" data-status="0" data-field="is_index" title="推荐在首页显示"><i class="Hui-iconfont">&#xe69e;</i></a>');
						}
						_this.remove();
						layer.msg(data['info'],{icon: 6,time:1000});
 ...
 * @param {Object} options 对象节点
 */
plugins.updateStar = function( options ) {
    ...
    $( document ).on( _opt.event, _opt.clickObj, function() {
        ...
        var _status = _this.attr( 'data-status' );
        var _field = _this.attr( 'data-field' );
        var confirmInfo = _status == 1 ? '确认要改为明星经销商吗？' : '确认要取消明星经销商吗？';
        layer.confirm(confirmInfo,function(index){
            $.ajax({
                url : jsObj['root'] + '/Admin/Member/commSetStatus/id/'+ _id +'/param/'+ _field +'-'+ _status +'.html?rnd='+ Math.random(),
                data : {},
                type : 'POST',
                async : true,
                dataType : 'json',
                ...
                success : function( data ) {
                    if ( data['status'] == 0 ) {
                        ...
                        if ( _status == 0 ) {
                            _this.parents("tr").find(".td-manage").prepend('<a href="javascript:void(0);" class="updateStar" data-id="'+ _id +'" data-status="1" data-field="is_star" title="改为明星经销商"><i class="Hui-iconfont">&#xe631;</i></a>');
                            _this.parents("tr").find(".td-status").html('<span class="label label-success radius">已改为明星经销商</span>');
                        } else if ( _status == 1 ) {
                            _this.parents("tr").find(".td-manage").prepend('<a href="javascript:void(0);" class="updateStar" data-id="'+ _id +'" data-status="0" data-field="status" title="取消明星经销商"><i class="Hui-iconfont">&#xe6e1;</i></a>');
                            _this.parents("tr").find(".td-status").html('<span class="label label-danger radius">已取消明星经销商</span>');
                        }
                        _this.remove();
                        layer.msg(data['info'],{icon: 6,time:1000});
                    }
